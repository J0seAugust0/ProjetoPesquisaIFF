...
if ($EnviarDocumento) {
	...
	if ($result_select->rowCount() > 0) {
		...
		header("Location: enviarArtigos.php");
	} 
	else {
		//Verificar se foi selecionado algum arquivo
		if(empty($nome_documento)||empty($_POST['titulo'])||empty($_POST['descricao'])){
			...
			//Inserir no BD
			$query_insert = "INSERT INTO documento (nome, titulo, descricao) VALUES (:nome, :titulo, :descricao)";
